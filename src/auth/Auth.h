...
struct RotatingSecrets {
  ...
  static void generate_test_instances(std::list<RotatingSecrets*>& ls) {
    ...
    ls.push_back(new RotatingSecrets);
    ls.back()->add(ls.front()->current());
    ls.back()->add(ls.front()->next());
    ls.back()->add(ls.front()->previous());
  }
};
