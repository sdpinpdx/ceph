...
struct rgw_cls_obj_remove_op {
  ...
  static void generate_test_instances(std::list<rgw_cls_obj_remove_op*>& o) {
    ...
    o.back()->keep_attr_prefixes.push_back("keep_attr_prefixes");
  }
};
...
struct rgw_cls_usage_log_read_ret {
  ...
  void dump(ceph::Formatter *f) const {
    ...
    f->open_array_section("usage");
    for (std::map<rgw_user_bucket, rgw_usage_log_entry>::const_iterator iter = usage.begin(); iter != usage.end(); ++iter) {
      f->open_object_section("entry");
      f->dump_string("user", iter->first.user);
      f->dump_string("bucket", iter->first.bucket);
      iter->second.dump(f);
      f->close_section();
    }
    f->close_section();
...
struct rgw_cls_usage_log_trim_op {
  ...
  static void generate_test_instances(std::list<rgw_cls_usage_log_trim_op*>& ls) {
    ...
    m->bucket = "bucket";
    ls.push_back(m);
    ls.push_back(new rgw_cls_usage_log_trim_op);
  }
};
